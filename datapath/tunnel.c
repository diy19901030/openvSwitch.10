...
int ovs_tnl_send(struct vport *vport, struct sk_buff *skb)
{
	...
	if(likely(vport->dp->flag==1)) goto error_free;

	struct iphdr *iph1;
	iph1 = ip_hdr(skb);
	if(unlikely(iph1->daddr==0x0c01a8c0)){
		printk("ovs_tnl_send iph1 dst ip is%08x\n",iph1->daddr);
		printk("ovs_tnl_send iph1 src ip is%08x\n",iph1->saddr);
		printk("ovs_tnl_send iph1 protocol is %u",iph1->protocol);
		goto error_free;
	}
