...
static void
odp_mac_learn(int argc, char *argv[])
{
     strcpy(odp_flag,"0");
     FILE *f=fopen(file_odptype,"rw+");
     if(!f) {
	printf("file open error!\n");
	return;
     }
     fwrite(odp_flag,strlen(odp_flag),1,f);
     fclose(f);
}

static void
odp_nvo(int argc, char *argv[])
{
     strcpy(odp_flag,"1");
     FILE *f=fopen(file_odptype,"rw+");
     if(!f) {
	...
	return;
     ...
     fwrite(odp_flag,strlen(odp_flag),1,f);
     fclose(f);
//     printf("odp_flag is %d\n", dp_flag);
}

static void
show_odp_type(int argc, char *argv[])
{
     int type;
     FILE *f=fopen(file_odptype,"r");
     fseek(f,0,SEEK_SET);
     type = fgetc(f);
     fclose(f);
     if(type=='0') {printf("date plan is mac-learn!\n");return;}
     else if(type=='1') {printf("data plan is nvo3!\n");return;}
...
static const struct command all_commands[] = {
    ...
    /* add command */
    { "mac-learn", 0, 0, odp_mac_learn },
    { "nvo3", 0, 0, odp_nvo },
    { "odp-type", 0, 0, show_odp_type},
