...
#define cmp_is_mac(a,b) ((a[0]-b[0])+(a[5]-b[5]))
...
struct flow_miss;
struct facet;
/* extend by chenyanming  */
uint8_t isis_mac[6]={0x01,0x80,0xc2,0x00,0x00,0x16};
/* original ISIS address */
uint8_t isis_mac_ori1[6]={0x01,0x80,0xc2,0x00,0x00,0x14};
uint8_t isis_mac_ori2[6]={0x01,0x80,0xc2,0x00,0x00,0x15};

unsigned char broadcast[6] = {0xff,0xff,0xff,0xff,0xff,0xff};
//uint32_t ttip=0x0002010a;
int path_flag;
path_flag = 0;
...
static void do_xlate_actions(const struct ofpact *, size_t ofpacts_len,
                             struct action_xlate_ctx *);
static void xlate_normal_original(struct action_xlate_ctx *);
static void xlate_normal(struct action_xlate_ctx *);
static void xlate_normal_all(struct action_xlate_ctx *);
...
flood_packets(struct action_xlate_ctx *ctx, bool all)
{
    ...
    HMAP_FOR_EACH (ofport, up.hmap_node, &ctx->ofproto->up.ports) {
         ...
         * functions: exclude the pactch port
         */
        patch_flag = strcmp(ofport->up.netdev->netdev_dev->netdev_class->type,"patch");
//	VLOG_INFO("flood_packets type is %s",ofport->up.netdev->netdev_dev->netdev_class->type);
//	VLOG_INFO("flood_packets name is %s",ctx->ofproto->up.name);
...
static void
flood_packets_sys(struct action_xlate_ctx *ctx)
{
    struct ofport_dpif *ofport;
    ...
    HMAP_FOR_EACH (ofport, up.hmap_node, &ctx->ofproto->up.ports) {
        ...
        if (!strcmp(ofport->up.netdev->netdev_dev->netdev_class->type,"patch")) {
                   continue;
               }
        if (!strcmp(ofport->up.netdev->netdev_dev->netdev_class->type,"vxlan")) {
            continue;
        } 
//	VLOG_INFO("flood_packets_sys type is %s",ofport->up.netdev->netdev_dev->netdev_class->type);
//	VLOG_INFO("flood_packets_sys of_port is %d",ofp_port);
	compose_output_action(ctx, ofp_port);
    }
...
/*
 * add by chenyanming
 * Function: flood_packet for arp replay,choose the inside port
 *
 */
static void
flood_packets_arp(struct action_xlate_ctx *ctx)
{
    struct ofport_dpif *ofport;
/* function for find outside port */
	uint16_t of_port_br;
    struct ofproto *ofproto;
	struct ofproto_port *ofproto_port;
	ofproto_port = (struct ofproto_port *)malloc(sizeof(struct ofproto_port));
	ofproto = &ctx->ofproto->up;
	port_query_by_name(ofproto,"eth1",ofproto_port);
	of_port_br = ofproto_port->ofp_port;
	free(ofproto_port);

		VLOG_INFO("flood_packets_sys of_port is %d",of_port_br);
...
//   VLOG_INFO("xlate_output_action src mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
//										(unsigned char)(ctx->flow.dl_src[0]),
//										(unsigned char)(ctx->flow.dl_src[1]),
//										(unsigned char)(ctx->flow.dl_src[2]),
//										(unsigned char)(ctx->flow.dl_src[3]),
//										(unsigned char)(ctx->flow.dl_src[4]),
//										(unsigned char)(ctx->flow.dl_src[5]));
//
//    VLOG_INFO("xlate_output_action port type is %x\n",port);
//
//    VLOG_INFO("xlate_output_action dst mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
//										(unsigned char)(ctx->flow.dl_dst[0]),
//										(unsigned char)(ctx->flow.dl_dst[1]),
//										(unsigned char)(ctx->flow.dl_dst[2]),
//										(unsigned char)(ctx->flow.dl_dst[3]),
//										(unsigned char)(ctx->flow.dl_dst[4]),
//										(unsigned char)(ctx->flow.dl_dst[5]));


    switch (port) {
    case OFPP_IN_PORT:
//	VLOG_INFO("OFPP_IN_PORT\n");
        compose_output_action(ctx, ctx->flow.in_port);
        break;
    case OFPP_TABLE:
//	VLOG_INFO("OFPP_TABLE\n");
        xlate_table_action(ctx, ctx->flow.in_port, 0, may_packet_in);
        break;
    case OFPP_NORMAL:
//	VLOG_INFO("OFPP_NORMAL\n");
	xlate_normal_all(ctx);
        break;
    case OFPP_FLOOD:
//	VLOG_INFO("OFPP_FLOOD\n");
                 ...
                 struct action_xlate_ctx *ctx)
{
	...
	/*
	 * Debug by chen yanming
	 */

//		VLOG_INFO("do_xlate_actions src mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
//												(unsigned char)(ctx->flow.dl_src[0]),
//												(unsigned char)(ctx->flow.dl_src[1]),
//												(unsigned char)(ctx->flow.dl_src[2]),
//												(unsigned char)(ctx->flow.dl_src[3]),
//												(unsigned char)(ctx->flow.dl_src[4]),
//												(unsigned char)(ctx->flow.dl_src[5]));
//		VLOG_INFO("do_xlate_actions ctx->type is %s\n",a->type);
//
//		VLOG_INFO("do_xlate_actions dst mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
//												(unsigned char)(ctx->flow.dl_dst[0]),
//												(unsigned char)(ctx->flow.dl_dst[1]),
//												(unsigned char)(ctx->flow.dl_dst[2]),
//												(unsigned char)(ctx->flow.dl_dst[3]),
...
//add by zzq---- find the port number of vxlan
uint32_t find_vxlan_odport(){

	uint32_t odp_port;
	char *type = "system";
	char *name = "ovs-system";
	struct dpif *dpif;
	int error;
...
//			VLOG_INFO("VM send port %u: %s", dpif_port.port_no, dpif_port.name);
			if(!strcmp(dpif_port.name, "vxlan_sys_4789")) odp_port = dpif_port.port_no;		
		}
	}
...
//add by cym---- find the port number of vxlan
uint32_t find_vnet_odport(const char *brname){
	...
	int error;
	uint16_t of_port;
	const struct ofproto_dpif *t_ofproto_dpif;

	error = dpif_open(name, type, &dpif);
	VLOG_INFO("find_vnet_odport VM send port %d\n", error);
	if(!error) {
		struct dpif_port dpif_port;
		struct dpif_port_dump dump;
		DPIF_PORT_FOR_EACH(&dpif_port, &dump, dpif){
			VLOG_INFO("find_vnet_odport VM send port %u: %s\n", dpif_port.port_no, dpif_port.name);
			if(!strcmp(dpif_port.name, "vnet16")) odp_port = dpif_port.port_no;
		}
	}
	t_ofproto_dpif = ofproto_dpif_lookup(brname);
	if(t_ofproto_dpif){

		of_port = odp_port_to_ofp_port(t_ofproto_dpif, odp_port);
		VLOG_INFO("find_vnet_odport VM send port %u \n", of_port);
	}
...
//add by zzq-----find the recv ofp_port of vm
uint16_t find_recv_ofport(const char *brname,unsigned char *mac){

	uint16_t of_port;
	const struct ofproto_dpif *t_ofproto_dpif;
	const struct mac_entry *e;
	uint32_t recv_odp_port;


	t_ofproto_dpif = ofproto_dpif_lookup(brname);
	if(t_ofproto_dpif) { 
//		VLOG_INFO("t_ofproto_dpif is not null\n");
		LIST_FOR_EACH(e, lru_node, &t_ofproto_dpif->ml->lrus){
			struct ofbundle *bundle = e->port.p;
			if(bundle){
//				VLOG_INFO("VM receive mac is "ETH_ADDR_FMT"\n", ETH_ADDR_ARGS(e->mac));
//		                VLOG_INFO("VM receive odp_port is %d\n", ofbundle_get_a_port(bundle)->odp_port);
				if(!cmp_mac(mac,e->mac)) recv_odp_port = ofbundle_get_a_port(bundle)->odp_port;
			}
		}
//		VLOG_INFO("VM receive recv_odp_port is %d\n", recv_odp_port);
		of_port = odp_port_to_ofp_port(t_ofproto_dpif, recv_odp_port);
//		VLOG_INFO("VM receive of_port1 is %d\n", of_port);
	}
	return of_port;
}

//add by cym----find the recv ofp_port ,which port type is patch
uint16_t find_recv_port(const char *brname){
...
//				VLOG_INFO("VM receive mac is "ETH_ADDR_FMT"\n", ETH_ADDR_ARGS(e->mac));
		       VLOG_INFO("VM receive odp_port is %d\n", ofbundle_get_a_port(bundle)->odp_port);
//				if(!cmp_mac(mac,e->mac)) recv_odp_port = ofbundle_get_a_port(bundle)->odp_port;
		       recv_odp_port = ofbundle_get_a_port(bundle)->odp_port;
			}
		}
		VLOG_INFO("VM receive recv_odp_port is %d\n", recv_odp_port);
		of_port = odp_port_to_ofp_port(t_ofproto_dpif, recv_odp_port);
...
static void 
xlate_normal_all(struct action_xlate_ctx *ctx)
{

//    VLOG_INFO("xlate_normal_all src ip is %08x\n",ctx->flow.nw_src);
//    VLOG_INFO("xlate_normal_all flow-type i is %x\n",ctx->flow.dl_type);
//    VLOG_INFO("xlate_normal_all dst ip is %08x\n",ctx->flow.nw_dst);


    if(ctx->flow.dl_type==0xff05) {
        VLOG_INFO("xlate_normal_all packet type is %x",ctx->flow.dl_type);
        xlate_normal_original(ctx);
    }
    else {    
	   int odp_type = get_odp_type();
           if(odp_type=='0') {
        	   xlate_normal_original(ctx);
//			    VLOG_INFO("odp_flag is %d\n", odp_flag);
			   }
		   else {
			   VLOG_INFO("xlate_normal_all src mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
													(unsigned char)(ctx->flow.dl_src[0]),
													(unsigned char)(ctx->flow.dl_src[1]),
													(unsigned char)(ctx->flow.dl_src[2]),
													(unsigned char)(ctx->flow.dl_src[3]),
													(unsigned char)(ctx->flow.dl_src[4]),
													(unsigned char)(ctx->flow.dl_src[5]));
			    VLOG_INFO("xlate_normal_all port type is %x\n",ctx->flow.dl_type);
			    VLOG_INFO("xlate_normal_all port type is %s\n",ctx->ofproto->up.name);
			    VLOG_INFO("xlate_normal_all dst mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
													(unsigned char)(ctx->flow.dl_dst[0]),
													(unsigned char)(ctx->flow.dl_dst[1]),
													(unsigned char)(ctx->flow.dl_dst[2]),
													(unsigned char)(ctx->flow.dl_dst[3]),
...
int cmp_mac(uint8_t mac1[], uint8_t mac2[]){
	int i;
//        int flag=0;
	for(i=0;i<6;i++){
		if(mac1[i]!=mac2[i]){
...
static void
xlate_normal_original(struct action_xlate_ctx *ctx)
{
    struct ofport_dpif *in_port;
    struct ofbundle *in_bundle;
    struct mac_entry *mac;
...
{
    struct ofport_dpif *in_port; /*the port which the packet from */
    struct ofbundle *in_bundle;
    struct mac_entry *mac;
    uint16_t vlan;
    uint16_t vid;
 ...
 //   VLOG_INFO("xlate_normal path_flag is %d\n", path_flag);
//    path_flag = 0;
    ...
    if (!strcmp(ctx->ofproto->up.name, "br0")){

     ctx->flag=0; 
/*
     if(!cmp_mac(b,ctx->flow.dl_dst)){

	   VLOG_INFO("IS-IS level 1 send\n");
	   ctx->port = 2;
           compose_output_action(ctx, ctx->port);
	   return;
     }
*/
     /* Determine output bundle. */
     mac = mac_learning_lookup(ctx->ofproto->ml, ctx->flow.dl_dst, vlan, &ctx->tags);
     /* find the input vn_ID */
     char* vm_vn_id = find_vnid(ctx->flow.dl_src);
     // find the remote vn according to the dl_src
     char *arp_flag = find_ip(ctx->flow.dl_src);

     /* Determine  how to deal with patch port */
//     char *patch = strcmp(in_port->up.netdev->netdev_dev->netdev_class->type,"patch");
     if (mac&&vm_vn_id==NULL) {
    	 VLOG_INFO("xlate_normal mac src is %02x:%02x:%02x:%02x:%02x:%02x\n",
    		 										(unsigned char)(ctx->flow.dl_src[0]),
    		 										(unsigned char)(ctx->flow.dl_src[1]),
    		 										(unsigned char)(ctx->flow.dl_src[2]),
    		 										(unsigned char)(ctx->flow.dl_src[3]),
    		 										(unsigned char)(ctx->flow.dl_src[4]),
    		 										(unsigned char)(ctx->flow.dl_src[5]));
	   VLOG_INFO("xlate_normal mac dst is %02x:%02x:%02x:%02x:%02x:%02x\n",
													(unsigned char)(ctx->flow.dl_dst[0]),
													(unsigned char)(ctx->flow.dl_dst[1]),
													(unsigned char)(ctx->flow.dl_dst[2]),
													(unsigned char)(ctx->flow.dl_dst[3]),
        ...
        if (mac->port.p != in_bundle) {
        	 VLOG_INFO("xlate_normal mac forwarding to learned port\n");
            xlate_report(ctx, "forwarding to learned port");
//	    struct ofbundle *temp_ofbundle=(struct ofbundle *)(mac->port.p);
//	    struct ofport_dpif *temp_ofport=ofbundle_get_a_port(temp_ofbundle);
//	    VLOG_INFO("after mac_learning_lookup ofp_port is %d\n", temp_ofport->up.ofp_port);
            output_normal(ctx, mac->port.p, vlan);
        } else {
        	 VLOG_INFO("xlate_normal mac learned port is input port, dropping\n");
            xlate_report(ctx, "learned port is input port, dropping");
        }
     }
  /* add by chenyanming */
     else if(vm_vn_id)
          {
    	    ...
    	    uint16_t of_port_br;
    	    char *br1 = find_br(vm_vn_id);

    	    struct ofproto *ofproto;
			struct ofproto_port *ofproto_port;
			ofproto_port = (struct ofproto_port *)malloc(sizeof(struct ofproto_port));
			ofproto = &ctx->ofproto->up;
			port_query_by_name(ofproto,"patch-to-p16",ofproto_port);
			of_port_br = ofproto_port->ofp_port;
			free(ofproto_port);

			VLOG_INFO("We find the vm of_port_br is: %d\n",of_port_br);
         	VLOG_INFO("We find the vm ID is: %s\n",vm_vn_id);
         	VLOG_INFO("We find the vm br_num is: %s\n",br1);


//         	of_port_br = find_recv_port("vnet16");
//         	ctx->port = of_port_br;
//         	of_port_br = find_vnet_odport(br1);
         	ctx->port = of_port_br;
         	compose_output_action(ctx, ctx->port);
//         	flood_packets(ctx,true);
//        	VLOG_INFO("We find the vm br_num is: %s\n",ctx->ofproto->up.name );
//           ctx->ofproto->up.name = br1;
//          	VLOG_INFO("We find the vm br_num is: %s\n",ctx->ofproto->up.name );
//            if(!cmp_mac(ctx->flow.dl_dst,broadcast)) flood_packets(ctx,true);


          }
//     else if(!patch){
//    	 xlate_report(ctx, "learned port is patch port, dropping");
//     }
     else if(arp_flag){

    	 flood_packets_arp(ctx);
     }
     else {
        struct ofbundle *bundle;

//	VLOG_INFO("mac is null\n");
/*
     if(!cmp_mac(b,ctx->flow.dl_dst)){
...
*/
	 ...
	 VLOG_INFO("xlate_normal send flow src mac is %02x:%02x:%02x:%02x:%02x:%02x\n",
	 										(unsigned char)(ctx->flow.dl_src[0]),
	 										(unsigned char)(ctx->flow.dl_src[1]),
	 										(unsigned char)(ctx->flow.dl_src[2]),
	 										(unsigned char)(ctx->flow.dl_src[3]),
	 										(unsigned char)(ctx->flow.dl_src[4]),
	 										(unsigned char)(ctx->flow.dl_src[5]));
	 VLOG_INFO("xlate_normal send flow dev_type is %s\n",
			 in_port->up.netdev->netdev_dev->netdev_class->type);

	//    if(!cmp_mac(isis_mac_ori1,ctx->flow.dl_dst)||!cmp_mac(isis_mac_ori2,ctx->flow.dl_dst)){

	//	   VLOG_INFO("IS-IS level 1 send\n");
//		     return;
//	      }

	flag=strcmp(in_port->up.netdev->netdev_dev->netdev_class->type,"vxlan");
	flag_patch = strcmp(in_port->up.netdev->netdev_dev->netdev_class->type,"patch");
	flag_VN =  find_vnid(ctx->flow.dl_src);
	if(flag_patch==0&&flag_VN==NULL)
	 {
	  VLOG_INFO("xlate_normal learned port is patch port and  have no VNInfo , dropping\n");
	  xlate_report(ctx, "learned port is patch port and  have no VNInfo , dropping");
	  return;
	 }
	else{
	 VLOG_INFO("xlate_normal learned port VNInfo is %s\n",flag_VN);
	}
...
//     flag=strcmp(in_port->up.netdev->netdev_dev->netdev_class->type,"vxlan");
     ...
     { 
           ...
           uint16_t of_port,t_ofp_port,of_port2,of_port4;
           of_port=0;
	   of_port4=1;
//	   t_ofp_port=3;
	   int vnid1 =0;
...
//	   VLOG_INFO("VM send key is %d\n", key);
//	   flag_isis=cmp_mac(isis_mac1,ctx->flow.dl_dst);
//	   VLOG_INFO("IS-IS send level 1 flag is %d\n",flag_isis);
/*   	   VLOG_INFO("send flow dst mac is %02x:%02x:%02x:%02x:%02x:%02x\n", 
										(unsigned char)(ctx->flow.dl_dst[0]),
										(unsigned char)(ctx->flow.dl_dst[1]),
										(unsigned char)(ctx->flow.dl_dst[2]),
										(unsigned char)(ctx->flow.dl_dst[3]),
...
*/
	   ...
	   if(!cmp_mac(ctx->flow.dl_dst,broadcast)) flood_packets(ctx,true);
		...
		br2 = ctx->ofproto->up.name;
	   	key = br_to_vnid(br2);
	   	ip = find_ip(ctx->flow.dl_dst);
//	   	VLOG_INFO("VM send br name is %s\n", br2);
//	   	key = find_key(ctx->flow.dl_dst);
//	   	VLOG_INFO("VM send ip is %s\n", ip);
		if((ip!=NULL)&&(key!=0)) {
		   t_odp_port = find_vxlan_odport();
//	   	   VLOG_INFO("VM send ip is %s\n", ip);
           	   of_port = tnl_find2(t_odp_port, ip, key);
 //         	   VLOG_INFO("VM send ofport is %d\n", of_port);
	  	   ctx->port = of_port;
//                 VLOG_INFO("VM send ctx->flow.tunnel.tun_id is %llx\n", ctx->flow.tunnel.tun_id);
           	   compose_output_action(ctx, ctx->port);
	        }
		
	        else {
/* the VMs belong to the same VN*/
		  vnid1 = find_vnid(ctx->flow.dl_dst);
		  if(vnid1) {
			br1 = find_br(vnid1);
//			VLOG_INFO("VM send vnid is %s\n", vnid1);
//			VLOG_INFO("VM send br is %s\n", br1);
			if(br1) {
				of_port2 = find_recv_ofport(br1, ctx->flow.dl_dst);
//				VLOG_INFO("VM send of_port2 is %d\n", of_port2);
				ctx->port = of_port2;
//          		VLOG_INFO("VM receive ctx->port is %d\n", ctx->port);
          			compose_output_action(ctx, ctx->port);	
			}
		  }
	        }
	...
	const struct ofproto_dpif *t_ofproto_dpif;
	const struct mac_entry *e;
	uint16_t of_port1,of_port3;
	of_port3=65534;
	/*
	 * add by chenyanming
	 * Function: deal with eth arp package
	 */
	uint16_t of_port_br;
	char *vn_arp_flag = find_ip(ctx->flow.dl_src);
	if(vn_arp_flag!=NULL)
	{
		struct ofproto *ofproto;
		struct ofproto_port *ofproto_port;
		ofproto_port = (struct ofproto_port *)malloc(sizeof(struct ofproto_port));
		ofproto = &ctx->ofproto->up;
		port_query_by_name(ofproto,"p16-to-p0",ofproto_port);
		of_port_br = ofproto_port->ofp_port;
		free(ofproto_port);
	...
	if(!cmp_mac(broadcast, ctx->flow.dl_dst)) {
		flood_packets(ctx, true);	
	}
	else {

//		vnid = find_vnid(ctx->flow.dl_dst);
//		VLOG_INFO("VM receive ctx->flow.tunnel.tun_id is %llx\n", ctx->flow.tunnel.tun_id);
		br = ctx->ofproto->up.name;

		if(br) {
			of_port1 = find_recv_ofport(br, ctx->flow.dl_dst);
//				VLOG_INFO("VM receive of_port1 is %d\n", of_port1);
			ctx->port = of_port1;
//          		VLOG_INFO("VM receive ctx->port is %d\n", ctx->port);
          		compose_output_action(ctx, ctx->port);	
                         ...
                         const char *argv[], void *aux OVS_UNUSED)
{
    ...
    ds_put_cstr(&ds, " port  VLAN         MAC             \n");
    LIST_FOR_EACH (e, lru_node, &ofproto->ml->lrus) {
        struct ofbundle *bundle = e->port.p;

        ds_put_format(&ds, "%5d  %4d  "ETH_ADDR_FMT"\n",
                      ofbundle_get_a_port(bundle)->odp_port,
